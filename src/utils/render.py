...
def render(viewpoint_camera, 
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = pc["features"].transpose(1, 2).view(-1, 3, (sh_degree+1)**2)
            dir_pp = (pc["means"] - viewpoint_camera.camera_center.repeat(pc["features"].shape[0], 1))
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)
        else:
            if separate_sh:
                dc, shs = pc["features_rgb"], pc["features_sh"]
            else:
                shs = pc["features"]
    else:
